...
const App: React.FC = () => {
  ...
  const handleProjectSelect = (project: Project) => {
    ...
    let firstIndex = 0;
    if (!isAdmin) {
      firstIndex = (Array.isArray(project?.updates) ? project.updates : []).findIndex(u => u?.status !== 'draft');
  ...
  const handleAddNewWeek = async () => {
    ...
    try {
        const latestWeek = (Array.isArray(activeProject?.updates) ? activeProject.updates : []).length > 0 ? Math.max(...(Array.isArray(activeProject?.updates) ? activeProject.updates : []).map(u => u?.weekNumber)) : 0;
        const newUpdate: WeeklyUpdate = {
            ...
            status: 'draft',
            stats: { completion: (Array.isArray(activeProject?.updates) ? activeProject.updates : [])[0]?.stats?.completion || 0, workersOnSite: 0, weatherConditions: 'Sunny' }
        };
        const updatedProject = { ...activeProject, updates: [newUpdate, ...(Array.isArray(activeProject?.updates) ? activeProject.updates : [])] };
...
<React.Fragment>
                    <div className="bg-white/5 border border-white/5 px-5 py-3 rounded-2xl backdrop-blur-sm flex items-center justify-between md:block w-full md:w-auto">
                        ...
                        <div className="flex items-center gap-6">
                           <div className="text-3xl md:text-4xl font-display font-extrabold tracking-tight text-white">{(Array.isArray(activeProject?.updates) ? activeProject.updates : [])[activeUpdateIndex]?.stats?.completion || 0}%</div>
                           <div className="w-20 md:w-24 h-1.5 md:h-2 bg-white/10 rounded-full overflow-hidden">
                              <div className="h-full bg-brand-blue transition-all duration-1000" style={{ width: `${(Array.isArray(activeProject?.updates) ? activeProject.updates : [])[activeUpdateIndex]?.stats?.completion || 0}%` }} />
                           </div>
                        </div>
                    </div>
</React.Fragment>
...
{/* Hero Experience Suite */}
                <div className={`grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-10 relative z-10 mb-4 md:mb-16`}>
...
<div className="flex flex-col gap-3 md:gap-8">
                           ...
                           <div id="splat-viewer" className="relative">
                              {heroTab === '3d' ? (
                                <SplatViewer type="3d" url={(Array.isArray(activeProject?.updates) ? activeProject.updates : [])[activeUpdateIndex]?.splatUrl} title="Polycam 3D Render" onFullScreenChange={setIsFullScreenMode} />
                              ) : (
                                <SplatViewer type="360" url={(Array.isArray(activeProject?.updates) ? activeProject.updates : [])[activeUpdateIndex]?.floorfyUrl} title="Floorfy 360 Tour" onFullScreenChange={setIsFullScreenMode} />
                              )}
                           </div>
                        </div>
)}
...
<>
                        <div id="media-gallery" className="pt-8 md:pt-10 border-t border-white/5">
                           ...
                           <MediaGrid 
                               media={(Array.isArray(activeProject?.updates) ? activeProject.updates : [])[activeUpdateIndex]?.media || []} 
                               onFullScreenChange={setIsFullScreenMode} 
                               isAdmin={isAdmin}
                               onMediaUpdate={(mediaId, updatedMedia) => {
                                   const newMedia = (Array.isArray(activeProject?.updates) ? activeProject.updates : [])[activeUpdateIndex]?.media?.map(m => m.id === mediaId ? updatedMedia : m);
                            ...
                            {isAdmin ? (
                                <div className="space-y-6">
                                    ...
                                    <div className="grid grid-cols-2 gap-6 pt-4 border-t border-white/5">
                                        ...
                                        <div><label className="text-[10px] text-slate-500 uppercase font-extrabold tracking-tight mb-2 block">Update Date</label><input type="date" className="w-full bg-brand-dark border border-white/5 shadow-2xl shadow-black/40 rounded-2xl px-6 py-3 text-sm text-white [color-scheme:dark]" value={(Array.isArray(activeProject?.updates) ? activeProject.updates : [])[activeUpdateIndex]?.date} onChange={e => handleUpdateField('date', e.target.value)} /></div>
                                    </div>
                                    <div>
                                    ...
                                    <div>
                                        ...
                                        <input 
                                            ...
                                            value={(Array.isArray(activeProject?.updates) ? activeProject.updates : [])[activeUpdateIndex]?.splatUrl || ''} 
                                    ...
                                    <div>
                                        ...
                                        <input 
                                            className="w-full bg-brand-dark border border-white/5 shadow-2xl shadow-black/40 rounded-2xl px-6 py-3 text-xs font-mono text-brand-blue" 
                                            value={(Array.isArray(activeProject?.updates) ? activeProject.updates : [])[activeUpdateIndex]?.floorfyUrl || ''} 
                                    ...
                                    <div><label className="text-[10px] text-slate-500 uppercase font-extrabold tracking-tight mb-2 block">Update Title</label><input type="text" className="w-full bg-brand-dark border border-white/5 shadow-2xl shadow-black/40 rounded-2xl px-6 py-3 text-sm text-white" value={(Array.isArray(activeProject?.updates) ? activeProject.updates : [])[activeUpdateIndex]?.title || ''} onChange={e => handleUpdateField('title', e.target.value)} /></div>
                                    <div><label className="text-[10px] text-slate-500 uppercase font-extrabold tracking-tight mb-2 block">Narrative</label><textarea className="w-full bg-brand-dark border border-white/5 shadow-2xl shadow-black/40 rounded-2xl px-6 py-3 h-32 resize-none text-sm text-white" value={(Array.isArray(activeProject?.updates) ? activeProject.updates : [])[activeUpdateIndex]?.summary} onChange={e => handleUpdateField('summary', e.target.value)} /></div>
                                    <div className="pt-4 border-t border-white/5">
                                    ...
                                    <div className="pt-4 border-t border-white/5">
                                        ...
                                        <div className="h-24"><WeatherWidget location={activeProject.location} date={(Array.isArray(activeProject?.updates) ? activeProject.updates : [])[activeUpdateIndex]?.date} /></div>
                                    </div>
                                </div>
                            ) : (
                                <div className="space-y-6 md:space-y-8">
                                    <div>
                                       <h2 className="text-2xl md:text-3xl font-display font-extrabold tracking-tight text-white leading-snug">{(Array.isArray(activeProject?.updates) ? activeProject.updates : [])[activeUpdateIndex]?.title}</h2>
                                       <p className="text-base text-slate-500 mt-4 leading-relaxed whitespace-pre-line">{(Array.isArray(activeProject?.updates) ? activeProject.updates : [])[activeUpdateIndex]?.summary || 'No summary notes for this week.'}</p>
                                    </div>
                                    <div className="grid grid-cols-2 gap-3 md:gap-6 pt-6 md:pt-8 border-t border-white/5">
                                        <WeatherWidget location={activeProject.location} date={(Array.isArray(activeProject?.updates) ? activeProject.updates : [])[activeUpdateIndex]?.date} />
...
<div className="bg-slate-900/50 border border-white/5 rounded-3xl overflow-hidden backdrop-blur-xl shadow-2xl transition-all duration-300">
                        ...
                        { user && (Array.isArray(activeProject?.updates) ? activeProject.updates : [])[activeUpdateIndex] && (
                            <div className="p-6 md:p-8 pt-0 border-t border-white/5">
                                <UpdateComments
                                    comments={(Array.isArray(activeProject?.updates) ? activeProject.updates : [])[activeUpdateIndex]?.comments || []}
                                    currentUser={user}
                    ...
                    <div className="bg-slate-900/50 border border-white/5 rounded-3xl overflow-hidden backdrop-blur-xl shadow-2xl transition-all duration-300">
                        ...
                        {isCalendarExpanded && (
                            <div className="p-6 md:p-8 pt-0 border-t border-white/5">
                                <ProjectCalendar 
                                    updates={isAdmin ? (Array.isArray(activeProject?.updates) ? activeProject.updates : []) : (Array.isArray(activeProject?.updates) ? activeProject.updates : []).filter(u => u?.status !== 'draft')}
                                    activeIndex={isAdmin ? activeUpdateIndex : (Array.isArray(activeProject?.updates) ? activeProject.updates : []).filter(u => u?.status !== 'draft').findIndex(u => u?.weekNumber === (Array.isArray(activeProject?.updates) ? activeProject.updates : [])[activeUpdateIndex]?.weekNumber)}
                                    onSelect={(idx) => {
                                        if (isAdmin) {
                                            setActiveUpdateIndex(idx);
                                        } else {
                                            const visible = (Array.isArray(activeProject?.updates) ? activeProject.updates : []).filter(u => u?.status !== 'draft');
                                            const originalIdx = (Array.isArray(activeProject?.updates) ? activeProject.updates : []).findIndex(u => u?.weekNumber === visible[idx]?.weekNumber);
