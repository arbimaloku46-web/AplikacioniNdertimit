...
export const MobileBottomNav: React.FC<MobileBottomNavProps> = ({ currentView, setCurrentView, text }) => {
  ...
  return (
    <div className="md:hidden fixed bottom-6 left-1/2 -translate-x-1/2 z-[100] w-[90%] max-w-sm pointer-events-none">
      <div className="flex items-center justify-around bg-slate-900/60 backdrop-blur-xl border border-white/10 rounded-3xl p-1.5 shadow-[0_8px_32px_rgba(0,0,0,0.4)] pointer-events-auto">
        <button 
        ...
        >
          <LayoutDashboard className={`w-5 h-5 mb-1 ${currentView === AppView.HOME ? 'text-brand-blue' : 'text-slate-300'}`} />
          <span className={`text-[10px] font-medium tracking-wider ${currentView === AppView.HOME ? 'text-white' : 'text-slate-400'}`}>
            Progress Suite
          </span>
        </button>
        <button 
        ...
        >
          <User className={`w-5 h-5 mb-1 ${currentView === AppView.PROFILE ? 'text-brand-blue' : 'text-slate-300'}`} />
          <span className={`text-[10px] font-medium tracking-wider ${currentView === AppView.PROFILE ? 'text-white' : 'text-slate-400'}`}>
            {text.profile}
          </span>
        </button>
      </div>
