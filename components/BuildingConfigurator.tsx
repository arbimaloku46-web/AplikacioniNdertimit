...
export const BuildingConfigurator: React.FC<BuildingConfiguratorProps> = ({ project, onSave, onClose }) => {
  ...
  const renderActiveImage = () => {
    ...
    return (
      <div className="flex-1 bg-slate-950 rounded-2xl flex flex-col items-center justify-center p-6 border border-white/5 shadow-inner min-h-[500px]">
        {imageUrl ? (
          <div className="relative inline-block w-full max-w-full rounded-xl overflow-hidden shadow-2xl">
            <img 
              ref={imgRef}
              src={imageUrl} 
              alt="Reference" 
              className={`w-full h-auto object-contain block select-none ${mode !== 'idle' ? 'cursor-crosshair' : ''}`}
              draggable={false}
            ...
            {mode !== 'idle' && (
              <div className="absolute top-4 left-4 right-4 flex justify-between items-center bg-slate-900/90 backdrop-blur-md p-3 rounded-xl border border-brand-blue/30 shadow-xl pointer-events-auto">
                <p className="text-white text-sm font-medium">{instruction}</p>
                <div className="flex gap-2">
                  <button onClick={undoLastPoint} disabled={points.length === 0} className="p-2 bg-slate-800 hover:bg-slate-700 rounded-lg text-white disabled:opacity-50"><Undo className="w-4 h-4" /></button>
                  ...
                  <button onClick={finishDrawing} className="px-4 py-2 bg-brand-blue hover:bg-blue-600 rounded-lg text-white font-bold flex items-center gap-2">
  ...
  return (
    <div className="w-full max-w-7xl mx-auto flex flex-col gap-6 text-white pb-20">
      <div className="flex items-center justify-between">
         <div>
            <h2 className="text-3xl font-display font-bold text-white mb-2">Building Configurator</h2>
            <p className="text-slate-400">Map out {project.name}'s interactive building experience.</p>
         </div>
         <div className="flex gap-3">
            <button onClick={onClose} className="px-4 py-2 bg-slate-800 hover:bg-slate-700 rounded-xl text-white font-medium transition-colors">Cancel</button>
            <button onClick={handleSave} className="px-6 py-2 bg-brand-blue hover:bg-blue-600 rounded-xl text-white font-bold transition-colors shadow-lg shadow-brand-blue/20 flex items-center gap-2">
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="w-full lg:w-96 shrink-0 flex flex-col gap-4">
          <div className="bg-slate-900 border border-white/5 rounded-2xl p-5 shadow-xl">
            <h3 className="font-bold mb-4 text-brand-blue">Main Building</h3>
            <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Main Image URL</label>
            <input 
              type="text" 
              value={buildingData.mainImageUrl}
              onChange={e => setBuildingData(prev => ({ ...prev, mainImageUrl: e.target.value }))}
              placeholder="https://..."
              className="w-full bg-slate-950 border border-white/10 rounded-lg p-3 text-sm text-white focus:border-brand-blue focus:ring-1 focus:ring-brand-blue outline-none transition-all"
            />
          </div>

          <div className="bg-slate-900 border border-white/5 rounded-2xl p-5 shadow-xl flex-1 overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
               <h3 className="font-bold text-brand-blue">Floors</h3>
               <button onClick={addFloor} className="p-1.5 bg-brand-blue/10 text-brand-blue hover:bg-brand-blue/20 rounded-md transition-colors"><Plus className="w-4 h-4" /></button>
            </div>
            
            <div className="space-y-3">
              {buildingData.floors.map(floor => (
                <div key={floor.id} className="border border-white/5 rounded-xl bg-slate-950 overflow-hidden">
                  <div 
                    className={`p-3 flex items-center justify-between cursor-pointer hover:bg-white/5 transition-colors ${activeFloorId === floor.id ? 'bg-white/5' : ''}`}
                  ...
                  {activeFloorId === floor.id && (
                    <div className="p-4 border-t border-white/5 space-y-4 bg-slate-900/50">
                      <div>
                        <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Floor Name</label>
                        <input type="text" value={floor.name} onChange={e => updateFloor(floor.id, { name: e.target.value })} className="w-full bg-slate-950 border border-white/10 rounded-lg p-2 text-xs text-white" />
                      </div>
                      <div>
                        <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Floor Plan Image URL</label>
                        <input type="text" value={floor.floorPlanUrl} onChange={e => updateFloor(floor.id, { floorPlanUrl: e.target.value })} className="w-full bg-slate-950 border border-white/10 rounded-lg p-2 text-xs text-white" />
                      </div>
                      <div className="flex gap-2">
                        <button 
                          onClick={() => { setMode('building'); setPoints([]); }}
                          disabled={!buildingData.mainImageUrl}
                          className="flex-1 py-2 bg-brand-blue/20 text-brand-blue hover:bg-brand-blue/30 rounded-lg text-xs font-bold transition-colors disabled:opacity-50"
                        >
                          {floor.svgPath ? 'Redraw Ext. Shape' : 'Draw Ext. Shape'}
                        </button>
                        <button onClick={() => deleteFloor(floor.id)} className="p-2 bg-red-500/10 text-red-400 hover:bg-red-500/20 rounded-lg transition-colors"><Trash2 className="w-4 h-4" /></button>
                      </div>

                      {/* Units */}
                      <div className="mt-4 pt-4 border-t border-white/5">
                        <div className="flex items-center justify-between mb-3">
                           <h4 className="text-xs font-bold text-slate-300">Units on {floor.name}</h4>
                        ...
                        <div className="space-y-2">
                          {floor.units.map(unit => (
                             <div key={unit.id} className="bg-slate-950 border border-white/10 rounded-lg p-3">
                               <div className="flex items-center justify-between mb-2">
                                  <input type="text" value={unit.name} onChange={e => updateUnit(floor.id, unit.id, { name: e.target.value })} className="bg-transparent border-none outline-none text-sm font-bold text-white w-2/3" />
                               ...
                               <input 
                                 ...
                                 className="w-full bg-slate-900 border border-white/5 rounded-md p-2 text-[10px] text-white mb-2" 
                               />
                               <select 
                                 value={unit.status} 
                                 onChange={e => updateUnit(floor.id, unit.id, { status: e.target.value as any })}
                                 className="w-full bg-slate-900 border border-white/5 rounded-md p-1.5 text-[10px] text-white mb-2 outline-none"
                               >
                               ...
                               <button 
                                 ...
                                 className="w-full py-1.5 bg-emerald-500/20 text-emerald-400 hover:bg-emerald-500/30 rounded-md text-[10px] font-bold transition-colors disabled:opacity-50"
                               >
