...
export const GlobalAuth: React.FC<GlobalAuthProps> = ({ onLogin, language, setLanguage }) => {
  ...
  return (
    <div className="min-h-screen w-full flex bg-brand-dark">
      ...
      <div className="hidden lg:flex w-1/2 relative overflow-hidden bg-slate-900/90 backdrop-blur-2xl">
        ...
        <div className="relative z-20 p-12 flex flex-col justify-between h-full text-white">
          <div>
            ...
            <h1 className="text-5xl font-display font-extrabold tracking-tight leading-tight mb-4 text-white">
              {text.appName} <br/>
          ...
          <div>
             <div className="glass-panel inline-block px-6 py-2 rounded-lg border border-brand-blue/30">
      ...
      <div className="w-full lg:w-1/2 flex flex-col items-center justify-center p-8 md:p-12 bg-brand-dark relative">
        
        {/* Language Toggle */}
        <div className="absolute top-6 right-6 flex items-center gap-2 z-30">
            <span className="text-slate-500 text-xs font-extrabold tracking-tight uppercase tracking-wider hidden sm:inline">{text.selectLanguage}:</span>
            <div className="flex bg-slate-900/90 backdrop-blur-2xl rounded-lg p-1 border border-white/5 shadow-2xl shadow-black/40">
                <button 
                    onClick={() => setLanguage('en')}
                    className={`px-3 py-1 rounded text-xs font-extrabold tracking-tight transition-all duration-300 ease-in-out ${language === 'en' ? 'bg-brand-blue text-white' : 'text-slate-500 hover:text-white'}`}
                >
                    EN
                </button>
                <button 
                    onClick={() => setLanguage('sq')}
                    className={`px-3 py-1 rounded text-xs font-extrabold tracking-tight transition-all duration-300 ease-in-out ${language === 'sq' ? 'bg-brand-blue text-white' : 'text-slate-500 hover:text-white'}`}
                >
                    SQ
                </button>
            </div>
        ...
        <div className="w-full max-w-md space-y-8 mt-12 md:mt-0">
          ...
          <div className="bg-slate-900/50 border border-white/5 rounded-2xl overflow-hidden shadow-2xl backdrop-blur-sm">
              <div className="p-8">
                  ...
                  <div className="space-y-4 animate-in fade-in zoom-in duration-300">
                        ...
                        <button
                            ...
                            disabled={isLoading}
                            className="w-full bg-white text-slate-900 hover:bg-slate-100 font-medium py-3 px-6 rounded-lg flex items-center justify-center gap-3 transition-all duration-300 ease-in-out disabled:opacity-70 disabled:cursor-not-allowed group relative overflow-hidden"
                        >
                        ...
                        <div className="relative flex py-2 items-center">
                            ...
                            <span className="flex-shrink-0 mx-4 text-slate-500 text-xs font-extrabold tracking-tight uppercase">OR</span>
                            <div className="flex-grow border-t border-slate-700"></div>
                        </div>
                        ...
                        <form onSubmit={handleClientSubmit} className="space-y-4">
                            ...
                            {mode === 'REGISTER' && (
                                <>
                                    <div>
                                        <label className="block text-xs font-extrabold tracking-tight text-slate-500 uppercase mb-1">{text.fullNameLabel}</label>
                                        <input 
                                            name="fullName"
                                            type="text"
                                            required
                                            ...
                                            className="w-full bg-slate-950 border border-white/5 shadow-2xl shadow-black/40 rounded-lg px-6 py-3 text-white focus:outline-none focus:border-brand-blue focus:ring-1 focus:ring-brand-blue transition-all"
                                            placeholder="Name Surname"
                                        />
                                    </div>
                                    <div>
                                        <label className="block text-xs font-extrabold tracking-tight text-slate-500 uppercase mb-1">{text.usernameLabel}</label>
                                        <input 
                                            name="username"
                                            type="text"
                                            required
                            ...
                            {/* Common Fields */}
                            <div>
                                <label className="block text-xs font-extrabold tracking-tight text-slate-500 uppercase mb-1">{text.identifierLabel}</label>
                                <input 
                                    name="identifier"
                                    type="text"
                                    required
                            ...
                            <div>
                                <label className="block text-xs font-extrabold tracking-tight text-slate-500 uppercase mb-1">{text.passwordLabel}</label>
                                <input 
                                    name="password"
                                    type="password" 
                                    required
                            ...
                            {mode === 'REGISTER' && (
                                <div>
                                    <label className="block text-xs font-extrabold tracking-tight text-slate-500 uppercase mb-1">{text.confirmPasswordLabel}</label>
                                    <input 
                                ...
                                <div className="text-red-400 text-center text-sm bg-red-500/10 p-3 rounded border border-red-500/20 animate-in fade-in slide-in-from-top-2">
                                    ...
                                    {showResend && (
                                        <button 
                                            ...
                                            className="mt-2 text-xs font-extrabold tracking-tight underline hover:text-red-300"
                                        >
                        ...
                        <div className="mt-4 text-center">
                             <p className="text-sm text-slate-500">
                                ...
                                <button 
                                    type="button"
                                    onClick={toggleMode}
                                    className="text-brand-blue font-extrabold tracking-tight hover:text-white transition-all duration-300 ease-in-out"
                                >
